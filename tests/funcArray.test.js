...
// the test syntax has been changed because I was getting the 
// The "actual" and "expected" arguments must be specified error
// 
...
it('returns undefined testh an empty array', ()=> {
  ...
  it('returns the correct array when having an array of the same element', () => {
    assert.deepStrictEqual(uniquifyArray(['FactoriaF5', 'FactoriaF5', 'FactoriaF5']), ['FactoriaF5']);
  });

  it('returns the same array when no element is repeated', () => {
    assert.deepStrictEqual(uniquifyArray(['Cat', 'Dog', 'Cow']), ['Cat', 'Dog', 'Cow']);
  });

  it('returns the uniquified array', () => {
    assert.deepStrictEqual(uniquifyArray(['iPhone', 'Samsung', 'Android', 'iOS', 'iPhone', 'Samsung', 'Nokia', 'Blackberry', 'Android']),
